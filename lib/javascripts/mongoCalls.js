...
var mongoCalls = {
  ...
  saveNewTxt: function(req, ta) {
    ...
    .then(function(user) {
      ...
      return textWriterEntries.insert({txt: ta, owner: req.session.username, contributors: []});
    });
  },

  // userCheck: function(req, redirect, next) {
  //   if(!req.session.username) {
  //     redirect();
  //   } else {
  //     textWriterAccounts.findOne({username: req.session.username}).then(function(user) {
  //         if(user) {
  //           next(user);
  //         } else {
  //           redirect();
