      ...
      return textWriterLikes.findOne({entryId: req.params.id});
        ...
        if(likedEntry === null) {
          return textWriterLikes.insert({entryId: req.params.id}
            ...
            console.log('here?');
            return textWriterAccounts.update({_id: tempObj.user._id}, {$push: {likeIds: tempObj.newLine._id, entriesLikedIds: tempObj.entry.entry._id}}, {upsert:"True"});
          }).then(function(accountUpdated) {
            return textWriterEntries.update({_id: tempObj.entry._id}, {$push: {likeIds: tempObj.newLike._id, likedByUserId: tempObj.user._id}}, {upsert:"True"});
          ...
          .then(function(oldL) {
            ...
            console.log(tempObj);
            // console.log(tempObj);
            return textWriterLikes.update({_id: tempObj.oldLike._id}, {$push: {likerIds: tempObj.user._id, entryId: tempObj.entry.entryId}});
          }).then(function(likesUpdated) {
