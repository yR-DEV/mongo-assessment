...
var mongoCalls = {
  ...
  saveNewTxt: function(req) {
    var txtFile = req.body.newTxtTA;
    console.log(txtFile);
    return textWriterEntries.find({}).then(function(entries) {
      return accountValidation.newTxt(req)
    }).then(function(errors) {
      if(errors) {
        return errors;
      } else if(errors === []) {
        txtFile.split('\n');
        return textWriterEntries.insert({entry: txtFile});
      }
    });
    console.log(txtFile);
  }
